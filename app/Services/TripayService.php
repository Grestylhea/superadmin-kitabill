...
class TripayService
{
     ...
     */
    public function validateCallbackSignature($callbackSignature, $rawBody)
    {
        // Validate using Raw Body (Standard Tripay)
        // Note: Do NOT use json_encode/ksort as it may differ from original body
        $signature = hash_hmac('sha256', $rawBody, $this->privateKey);

        return hash_equals($signature, (string) $callbackSignature);
    }
    ...
    public function handleCallback($request)
    {
        ...
        Log::info('Tripay Callback Received', [
            ...
            'data_id' => $data['reference'] ?? 'N/A',
            'status' => $data['status'] ?? 'N/A'
        ]);

        // Validate signature
        if (!$this->validateCallbackSignature($callbackSignature, $rawBody)) {
            Log::error('Tripay Callback Invalid Signature', [
                'received' => $callbackSignature,
                'calculated_sample' => hash_hmac('sha256', $rawBody, $this->privateKey)
            ]);
