    ...
    public function store(Request $request)
{
    ...
    // =========================================================
    // 1. Jika invoice terkait ke Package → hitung due_date otomatis
    // =========================================================
    if (!empty($validated['package_id'])) {
        $package = Package::find($validated['package_id']);

        if ($package) {
            // issue_date dari form
            $issueDate = Carbon::parse($validated['issue_date']);

            // gunakan helper calculateExpiry() yang sudah kamu buat di bawah
            $due = $this->calculateExpiry($package, $issueDate);

            // timpa due_date dari form dengan hasil perhitungan
            $validated['due_date'] = $due;

            // Kalau period belum diisi, isi otomatis (opsional, boleh dihapus)
            if (empty($validated['period'])) {
                $validated['period'] = $issueDate->format('d M Y') . ' - ' . $due->format('d M Y');
            }
        }
    }
    // Catatan:
    // - Jika package_id kosong → due_date tetap pakai input manual user.
    // - Validasi tetap jalan karena form tetap kirim due_date (bisa isi sama issue_date dulu).

    // =========================================================
    // 2. Hitung subtotal, pajak, diskon, total (kode kamu yang lama)
    // =========================================================
    $items = $request->items ?? [];
    $subtotal = 0;
    ...
    // =========================================================
    // 3. Generate nomor invoice (pakai format timestamp kamu)
    // =========================================================
    $prefix    = 'INV-' . date('Ym');
    ...
    // =========================================================
    // 5. Log activity (kode kamu yang lama)
    // =========================================================
    ActivityLog::log(
        'created',
        'Invoice',
...
/**
 * Hitung tanggal jatuh tempo invoice berdasarkan paket & tanggal mulai.
 *
 * - Monthly + custom_expire_day → selalu jatuh tempo di tanggal tersebut.
 * - Weekly → 7 hari dari startDate (jam pakai custom_expire_time kalau ada).
 */
protected function calculateExpiry(Package $package, Carbon $startDate): Carbon
{
    $time = $package->custom_expire_time ?: '23:59:00';

    if ($package->billing_cycle === 'Monthly') {

        if ($package->custom_expire_day) {
            $day = $package->custom_expire_day;

            // tanggal calon expired bulan ini
            $candidate = Carbon::create(
                $startDate->year,
                $startDate->month,
                min($day, $startDate->daysInMonth),
                substr($time, 0, 2),
                substr($time, 3, 2),
                0
            );

            // kalau sudah lewat / sama dengan startDate → bulan depan
            if ($candidate->lessThanOrEqualTo($startDate)) {
                $candidate->addMonth();
        ...
        // kalau tidak ada custom_expire_day
        return $startDate->copy()->addMonth()->setTimeFromTimeString($time);
    }

    if ($package->billing_cycle === 'Weekly') {
        return $startDate->copy()->addDays(7)->setTimeFromTimeString($time);
    }

    // fallback, misal Daily atau lainnya → +1 hari
    return $startDate->copy()->addDay()->setTimeFromTimeString($time);
}
