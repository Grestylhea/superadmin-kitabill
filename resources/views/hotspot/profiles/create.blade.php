...
@section('title', 'Add User Profile - ' . $router->name)
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h3>
                        <i class="fa fa-plus"></i> Add User Profile
                    </h3>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form method="POST" action="{{ route('hotspot.profiles.store') }}" autocomplete="off">
                        @csrf
                        ...
                        <table class="table">
                            <tr>
                                ...
                                <td>
                                    <input class="form-control" type="text" name="name" value="{{ old('name') }}" required autofocus onchange="remSpace(this)">
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <select class="form-control" name="address_pool">
                                        ...
                                        @foreach($pools as $pool)
                                            <option value="{{ $pool['name'] }}" {{ old('address_pool') == $pool['name'] ? 'selected' : '' }}>
                                                {{ $pool['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input class="form-control" type="number" name="shared_users" value="{{ old('shared_users', 1) }}" min="1" required>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">Rate limit [up/down]</td>
                                <td>
                                    <input class="form-control" type="text" name="rate_limit" value="{{ old('rate_limit') }}" placeholder="Example: 512k/1M">
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <select class="form-control" name="expired_mode" id="expmode" onchange="toggleValidity()" required>
                                        ...
                                        <option value="0" {{ old('expired_mode') == '0' ? 'selected' : '' }}>None</option>
                                        <option value="rem" {{ old('expired_mode') == 'rem' ? 'selected' : '' }}>Remove</option>
                                        <option value="ntf" {{ old('expired_mode') == 'ntf' ? 'selected' : '' }}>Notice</option>
                                        <option value="remc" {{ old('expired_mode') == 'remc' ? 'selected' : '' }}>Remove & Record</option>
                                        <option value="ntfc" {{ old('expired_mode') == 'ntfc' ? 'selected' : '' }}>Notice & Record</option>
                                    </select>
                                </td>
                            </tr>
                            <tr id="validity-row" style="display:none;">
                                <td class="align-middle">Validity</td>
                                <td>
                                    <input class="form-control" type="text" name="validity" id="validity" value="{{ old('validity') }}" placeholder="Example: 8h, 30d, 7d">
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input class="form-control" type="number" name="selling_price" value="{{ old('selling_price') }}" min="0" step="0.01">
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <select class="form-control" name="lock_user" required>
                                        <option value="Disable" {{ old('lock_user', 'Disable') == 'Disable' ? 'selected' : '' }}>Disable</option>
                                        <option value="Enable" {{ old('lock_user') == 'Enable' ? 'selected' : '' }}>Enable</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <select class="form-control" name="parent_queue">
                                        ...
                                        @foreach($queues as $queue)
                                            <option value="{{ $queue['name'] }}" {{ old('parent_queue') == $queue['name'] ? 'selected' : '' }}>
                                                {{ $queue['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
