    ...
    <div class="col-lg-8">
        <div class="custom-table mb-4">
            ...
            @if($package->custom_expire_day)
                <p>
                    Expired tiap tanggal {{ $package->custom_expire_day }}
                    @if($package->custom_expire_time)
                        jam {{ $package->custom_expire_time }}
                    @endif
                </p>
            @endif
        ...
        <div class="custom-table mb-4">
            <h6 class="fw-bold mb-3">Router & Tipe Koneksi</h6>
            <table class="table table-borderless">
                @foreach($package->routers as $router)
                <tr>
                    <td width="200" class="fw-semibold">
                        @if($loop->first)
                            Router
                        @endif
                    </td>
                    <td>:
                        ...
                        <span class="badge bg-info">
                            {{ strtoupper($router->pivot->connection_type) }}
                        </span>
                    </td>
                </tr>
                @endforeach
