                ...
                <form action="{{ route('routers.test', $router) }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-outline-success w-100">
                        <i class="bi bi-wifi"></i> Test Connection
                                ...
                                <button type="button" onclick="runFullImport({{ $router->id }})" class="btn btn-primary w-100 mb-2">
                    <i class="bi bi-cloud-download"></i> Import Mikrotik (Full)
                </button>
                <a href="{{ route('routers.ssh-terminal', ) }}" class="btn btn-dark">
...
@push('scripts')
<script>
    function runFullImport(routerId) {
        Swal.fire({
            title: 'Importing...',
            text: 'This process involves Syncing PPPoE Users, Profiles, and Hotspot Profiles. Please wait.',
            icon: 'info',
        ...
        fetch(`/routers/${routerId}/import-mikrotik`, {
            method: 'POST',
            headers: {
        ...
        .then(({ status, body }) => {
            if (status >= 200 && status < 300) {
                Swal.fire({
                    ...
                    html: `<pre class="text-start">${body.message}</pre>`,
                    icon: 'success'
                }).then(() => {
                    location.reload();
                });
