...
<!-- Realtime Customer Status (Online / Offline / Suspended) -->
<script>
document.addEventListener('DOMContentLoaded', function () {
    const customerMarkers = {}; // Simpan marker pelanggan biar bisa diupdate
        ...
        fetch('/api/customers/status')
            ...
            .then(data => {
                ...
                data.data.forEach(customer => {
                    const key = `cust_${customer.id}`;
                    const color = customer.color;
                    const statusText = customer.isOnline ? 'Online 🟢' :
                        (customer.status === 'suspended' ? 'Suspended 🟠' : 'Offline 🔴');

                    // Hapus marker lama
                    if (customerMarkers[key]) {
                        networkMap.removeLayer(customerMarkers[key]);
                    }

                    // Icon custom warna status
                    const customerIconDynamic = L.divIcon({
                        className: 'custom-marker',
                        html: `
                            <div style="
                                background: ${color};
                            ...
                            ">
                                <i class="bi bi-house text-white" style="font-size: 10px;"></i>
                            </div>
                        `,
                        iconSize: [22, 22],
                        iconAnchor: [11, 11]
                    ...
                    // Tambah marker baru
                    const marker = L.marker([customer.lat, customer.lon], { icon: customerIconDynamic })
                        .bindPopup(`
                            <div class="p-2">
                                <h6 class="fw-bold mb-2"><i class="bi bi-house"></i> ${customer.name}</h6>
                                <p class="mb-1 small"><strong>Status:</strong> ${statusText}</p>
                                <p class="mb-1 small"><strong>Koneksi:</strong> ${customer.status}</p>
                                <a href="/customers/${customer.id}/edit" class="btn btn-sm btn-secondary mt-2 w-100">View Details</a>
                            </div>
                        `)
                        .addTo(layers.customers);

                    customerMarkers[key] = marker;
                });
            })
            .catch(err => console.error('Gagal memuat status customer:', err));
    }
